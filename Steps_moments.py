...
def create_multiIndex(dimensions: list, names: list) -> pd.core.indexes.multi.MultiIndex:
    if len(dimensions) != len(names):
        ...
        mIndex = pd.MultiIndex.from_product([arrays[k] for k in range(len(dimensions))],
...
def get_first_spike_per_phase(cell: int, dataframe_spikes: pd.core.frame.DataFrame, stimulus_traits: dict,
                              phase_dur: int, sampling_freq: float, ON_thres: float) -> list:
    ...
    for trial in range(stimulus_traits['stim_trials']):
        ...
        for repeat in range(stimulus_traits['stim_repeats']):
            ...
            try:
                ...
                first_spikes_s_OFF[repeat] = spikes_per_trial[repeat][next(
                    i for i, v in enumerate(spikes_per_trial[repeat]) if
                    v > ON_thres * sampling_freq)] / sampling_freq - step_dur
...
def get_number_of_spikes_per_phase(cell: int, dataframe_spikes: pd.core.frame.DataFrame, stimulus_traits: dict,
                                   phase_dur: int, sampling_freq: float, ON_thres: float) -> list:
    """
    ...
    for trial in range(stimulus_traits['stim_trials']):
        ...
        for repeat in range(stimulus_traits['stim_repeats']):
            try:
                ...
                nr_of_spikes[repeat] = next(
                    k for k, v in enumerate(spikes_per_trial[repeat]) if v > ON_thres * sampling_freq)
...
def apply_ON_correction(cell: int, dataframe_spikes: pd.core.frame.DataFrame, first_spike: list, nspikes: list,
                        stimulus_traits: dict, ON_thres: int, sampling_freq: float, ) -> list:
    """
    ...
    to_replace = [
        dummy[[first_spike[trial][repeat] - 0.75 * reference < 0 for repeat in range(stimulus_traits['stim_repeats'])]]
        for trial in range(stimulus_traits['stim_trials'])]
    ...
    for trial in range(stimulus_traits['stim_trials']):
        ...
        if len(to_replace[trial]) == 0:
            ...
            spikes_per_trial = (
            sas.get_spikes_per_trigger_type_new(cell_spikes, stimulus_traits['stim_rel_trig'], trial,
                                                stimulus_traits['stim_trials'])[0])
...
def apply_ON_correction_spiketrain(spiketrain: list, first_spike: list, nspikes: list, stimulus_traits: dict,
                                   ON_thres: int, sampling_freq: float, ) -> list:
    """
    Currently deprecated
    """
...
def calculate_moments(nmoments: int, stimulus_traits: dict, first_spike: list, nspikes: list) -> list:
    ...
    for phase in range(len(first_spike)):
        ...
        for trial in range(stimulus_traits['stim_trials']):
            moments_overview_temp[0, trial] = (np.count_nonzero(~np.isnan(first_spike[phase][trial])) / stimulus_traits[
                'stim_repeats'])  # reliability
            moments_overview_temp[1, trial] = (np.nanstd(
                first_spike[phase][trial]))  # [~np.isnan(first_spike[phase][trial])]))                   #precision
            moments_overview_temp[2, trial] = (np.nanmean(
                first_spike[phase][trial]))  # [~np.isnan(first_spike[phase][trial])]))                  #latency
            moments_overview_temp[3, trial] = (np.nanmean(nspikes[phase][trial]))  # nrofspikes
            moments_overview_temp[4, trial] = np.round(
                np.nanvar(np.where(nspikes[phase][trial] != 0, nspikes[phase][trial], np.nan), 0), 2)  # varnrofspikes
...
def populate_multistimulus_df(dataframe: pd.core.frame.DataFrame, position: int, stim_position: int,
                              values: list, ) -> pd.core.frame.DataFrame:
    for row in range(values.shape[0]):
...
def calculate_ONOFF_polarity(ONOFF_dfr: list) -> np.ndarray:
    ...
    """
    return ((ONOFF_dfr[0].loc(axis=0)[:, 2]['Mean_nr'] - ONOFF_dfr[1].loc(axis=0)[:, 2]['Mean_nr']) / (
                ONOFF_dfr[0].loc(axis=0)[:, 2]['Mean_nr'] + ONOFF_dfr[1].loc(axis=0)[:, 2]['Mean_nr'])).values
...
def get_505_spikes(cell_idx, df_spikes, stimulus_traits, into='frames', sampling_freq=None, inone=True):
    ...
    spikes_per_trial = (sas.get_spikes_per_trigger_type_new(cell_spikes, stimulus_traits['stim_rel_trig'], 2,
                                                            stimulus_traits['stim_trials'])[0])
    if into == 'time':
        if inone == True:
...
def calculate_transcience(object, ON_edges, OFF_edges):
    ...
    for cell in range(len(object.cell_idces)):
        ...
        take_hist = \
        np.histogram(get_505_spikes(object.cell_idces[cell], object.df_spikes, object.stimulus_traits), bins=200,
                     range=[0, 4 * object.sampling_freq])[0]
        ...
        if Basic.helper_filter(object.df_ON.loc(axis=0)[cell, 2]['Latency'], ON_edges[0][1] * 0.02, 'bigger'):
            if bool(Basic.helper_filter(object.df_ON.loc(axis=0)[cell, 2]['Reliability'], .79,
                                        'bigger') * Basic.helper_filter(object.df_ON.loc(axis=0)[cell, 2]['Precision'],
                                                                        .18, 'smaller')):
        ...
        ON_binS = np.array(ON_edges[1]) + incr if (np.array(ON_edges[1]) + incr)[1] < 100 else np.array(
            [ON_edges[1][0] + incr, ON_edges[1][1]])

        if Basic.helper_filter(object.df_OFF.loc(axis=0)[cell, 2]['Latency'], (OFF_edges[0][1] * 0.02) - 2, 'bigger'):
            if bool(Basic.helper_filter(object.df_OFF.loc(axis=0)[cell, 2]['Reliability'], .79,
                                        'bigger') * Basic.helper_filter(object.df_OFF.loc(axis=0)[cell, 2]['Precision'],
                                                                        .18, 'smaller')):
                incr = int(
                    np.floor((object.df_OFF.loc(axis=0)[cell, 2]['Latency'] + 2 - OFF_edges[0][0] * 0.02) / 0.02))
                # print(incr)
            else:
        ...
        with warnings.catch_warnings():
            ...
            try:
                ON_profile = (sum(take_hist[ON_binT[0]:ON_binT[1]]) / 4 - sum(
                    take_hist[ON_binS[0]:ON_binS[1]]) / np.diff(ON_binS)) / (
                                         sum(take_hist[ON_binT[0]:ON_binT[1]]) / 4 + sum(
                                     take_hist[ON_binS[0]:ON_binS[1]]) / np.diff(ON_binS))
            except:
                ON_profile = 0
            try:
                OFF_profile = (sum(take_hist[OFF_binT[0]:OFF_binT[1]]) / 4 - sum(
                    take_hist[OFF_binS[0]:OFF_binS[1]]) / np.diff(ON_binS)) / (
                                          sum(take_hist[OFF_binT[0]:OFF_binT[1]]) / 4 + sum(
                                      take_hist[OFF_binS[0]:OFF_binS[1]]) / np.diff(ON_binS))
            except:
                OFF_profile = 0
            ...
            if 'ONOFF_profile' not in locals():
                ...
                try:
                    ONOFF_profile = ((sum(take_hist[ON_binT[0]:ON_binT[1]]) + sum(take_hist[ON_binS[0]:ON_binS[1]])) - (
                                sum(take_hist[OFF_binT[0]:OFF_binT[1]]) + sum(take_hist[OFF_binS[0]:OFF_binS[1]]))) / ((
                                                                                                                                   sum(take_hist[
                                                                                                                                       ON_binT[
                                                                                                                                           0]:
                                                                                                                                       ON_binT[
...
class Steps_505():
    ...
    def __init__(self, recording, json_dict: str, FFF_index, sampling_freq):
        ...
        self.transience, _ = Basic.proceed(calculate_transcience,
                                           (self, [[11, 15], [17, 100]], [[111, 115], [117, 200]]))

        plot_polVStrans([self.polarity, self.transience,
                         Basic.create_color_book([self.ON_bounds, self.OFF_bounds, self.ONOFF_bounds],
                                                 ['red', 'gray', '#eedbdb'])], [-1.04, 1.04])
        ...
        with warnings.catch_warnings():
            ...
            for cell in range(len(self.cell_idces)):
                ...
                if '4' in self.stimulus_traits['stim_name']:
                    first_spike_test_cell = get_first_spike_per_phase(self.cell_idces[cell], self.df_spikes,
                                                                      self.stimulus_traits, 2 * 4, self.sampling_freq,
                                                                      2 * 2.0)
                    nspikes_test_cell = get_number_of_spikes_per_phase(self.cell_idces[cell], self.df_spikes,
                                                                       self.stimulus_traits, 2 * 4, self.sampling_freq,
                                                                       2 * 2.0)
                else:
                    first_spike_test_cell = get_first_spike_per_phase(self.cell_idces[cell], self.df_spikes,
                                                                      self.stimulus_traits, 4, self.sampling_freq, 2.0)
                    nspikes_test_cell = get_number_of_spikes_per_phase(self.cell_idces[cell], self.df_spikes,
                                                                       self.stimulus_traits, 4, self.sampling_freq, 2.0)
                ...
                moments_ON_OFF = calculate_moments(len(self.df_ON.columns), self.stimulus_traits, first_spike_test_cell,
                                                   nspikes_test_cell)
