...
class Moving_stimulus():
    ...
    def __init__(self, dataframe_spikes, dataframe_stimulus, directions_degrees, stationary=False):
        ...
        self.polarplots = self.plot_polar_pref_selection(np.arange(self.normed_spikes_per_segment.shape[0])[
                                                             np.logical_or(self.masking_array,
                                                                           self.ds_vals > self.ds_cutoff)])
            ...
            for idx in range(self.df_spikes.shape[0]):
                for trigger in range(self.repeat_logic):
                    ...
                    spikes_per_dir = (sas.get_spikes_per_trigger_type_new(
                        self.df_spikes.loc[idx]['Spikes'].compressed(),
                        ((self.df_stimulus['Trigger_Fr_relative'].values)[0]),
                        trigger, self.repeat_logic)[0])

                    self.spikes_per_direction[idx, trigger] = sum(
                        [len(listElem) for listElem in spikes_per_dir]) / self.nr_trials

                    for trial in range(self.nr_trials):
                        self.spikes_per_segment[trial, idx, trigger] = len(spikes_per_dir[trial])

                self.spikes_per_direction[idx] = self.reorder(self.spikes_per_direction[idx], self.re_order,
                                                              len(self.spikes_per_direction[idx]))

                for trial in range(self.nr_trials):
                    self.spikes_per_segment[trial, idx, :] = self.reorder(self.spikes_per_segment[trial, idx, :],
                                                                          self.re_order,
                                                                          len(self.spikes_per_segment[trial, idx, :]))

        self.spikes_per_segment = np.append(self.spikes_per_segment, self.spikes_per_segment[:, :, None, 0], axis=2)

        self.spikes_per_direction = np.insert(self.spikes_per_direction, self.spikes_per_direction.shape[1],
                                              self.spikes_per_direction[:, 0], axis=1)
    ...
    def cell_hist_per_dir(self, cell, nr_bins, trial_dur, sampling_freq=17852.76785):
        ...
        for trigger in range(self.repeat_logic):
            ...
            hist_per_dir[trigger, :] = \
            np.histogram(np.sort([val / sampling_freq for sublist in spikes_per_dir for val in sublist]), bins=nr_bins,
                         range=[0, trial_dur])[0]
            ...
            for row in range(spike_profile.shape[0]):
                ...
                for point in points:
                    ...
                    if point.born == None:
                        point.born = 0
                    if (time_profile[point.born] > t_thresh) & (
                            spike_profile[row][point.born] - spike_profile[row][point.died] > sig_thresh):
                        idces.append(True)
                    else:
                        idces.append(False)
    ...
    def remove_silent(self, nbins=12, phase_dur=4, t_thresh=1, sig_thresh=4):
        ...
        """
        print(
            'Assuming default threshold params for a 1000speed stimulus\nFirst removing the silent (no detected events) neurons\n****')
        silent_neurons = []
        for cell_idx in np.arange(len(self.df_spikes)):
            ...
            if sum(np.unique(
                    [len(self.find_threshold_events(spikes_profile, t_thresh=t_thresh, sig_thresh=sig_thresh)[i]) for i
                     in range(self.repeat_logic)])) < 1:
    ...
    def norm_byarea(self):
        ...
        for cell in range(self.spikes_per_segment.shape[1]):
            self.normed_spikes_per_segment[cell] = np.nanmean(self.spikes_per_segment[:, cell], axis=0) / (
                        np.nansum(self.spikes_per_segment[:, cell]) / (self.nr_trials * self.repeat_logic))
    ...
    def norm_byarea_p(self, binned_spikes, n_trials, n_directions):
        ...
        for cell in range(binned_spikes.shape[1]):
            normed_spikes_per_segment[cell] = np.mean(binned_spikes[:, cell], axis=0) / (
                        np.sum(binned_spikes[:, cell]) / (n_trials * n_directions))
    ...
    def find_vector_angle_neighbors(self, vector_angle, n_neighbors=3):
        ...
        if n_neighbors == 0:
            ...
            return [centre,
                    [centre - 1, centre + 1][np.argmin(np.abs(np.array(centre - 1, centre + 1) - vector_angle))]]
    ...
    def calculate_dos(self, ):
        ...
        """
        nspikes_arrtoy = np.zeros(
            [self.normed_spikes_per_segment.shape[0], self.normed_spikes_per_segment.shape[1] - 1])
    ...
    def calculate_dsi_significance(self, npermutations=1000, ntrials=10, ndirections=8, a=0.05, plot_example=True):
        ...
        for real_neuron in range(self.normed_spikes_per_segment.shape[0]):
            ...
            if sum(self.calculate_dos_p(self.norm_byarea_p(permuted_pop, ntrials, ndirections), has_9th=False)[0] >
                   self.ds_vals[real_neuron]) / npermutations < a:
                self.dsi_sig[real_neuron] = True
                if plot_example == True:
                    fig = px.histogram(
                        self.calculate_dos_p(self.norm_byarea_p(permuted_pop, ntrials, ndirections), has_9th=False)[0],
                    ...
                    fig.update_layout(height=500, width=400, showlegend=False,
                                      title_text="Example of Significant DS cell <br> with its permuted DS distribution",
                                      title_x=0.5)
    ...
    def plot_ds_onnull(self, ):
        ...
        fig.add_trace(go.Scatter(x=self.ds_vals[np.arange(len(self.ds_vals))[self.dsi_sig]],
                                 y=[sum(self.dsi_sig) / len(self.ds_vals)] * sum(self.dsi_sig), mode='markers',
                                 name='significant DS cells'))
    ...
    def plot_hidden_gems(self, ):
        ...
        self.hidden_gems = np.setdiff1d(np.arange(len(self.dsi_sig))[self.dsi_sig], np.arange(len(self.dsi_sig))[
            np.logical_or(self.masking_array, self.ds_vals > self.ds_cutoff)])
    ...
    def plot_polar_histogram(self, criterion, threshold, title_name):
        ...
        fig.add_trace(go.Barpolar(
            r=np.histogram(self.vector_angles[~masking_array], bins=10, range=[0, 360],
                           weights=criterion[~masking_array])[0],
            theta=np.histogram(self.vector_angles[~masking_array], bins=10, range=[0, 360],
                               weights=criterion[~masking_array])[1][:-1],
            width=360 / 10,
        ...
        fig.add_trace(go.Barpolar(
            r=
            np.histogram(self.vector_angles[masking_array], bins=10, range=[0, 360], weights=criterion[masking_array])[
                0],
            theta=
            np.histogram(self.vector_angles[masking_array], bins=10, range=[0, 360], weights=criterion[masking_array])[
                1][:-1],
        ...
        for y in range(0, nr_rows):
            for x in range(0, nr_cols):
                ...
                if y * nr_cols + x < len(select):
                    ...
                    if norm == True:
                        ...
                        for trial in range(self.spikes_per_segment.shape[0]):
                            fig.add_trace(go.Scatterpolar(
                                r=self.spikes_per_segment[trial, select[y * nr_cols + x]] / (
                                            np.sum(self.spikes_per_segment[:, select[y * nr_cols + x]]) / 80),
                    ...
                    fig.add_trace(go.Scatterpolar(
                        r=[0, pycircstat.resultant_vector_length(self.angles_ordered_a[:-1],
                                                                 self.normed_spikes_per_segment[
                                                                     select[y * nr_cols + x]][:-1], d=0.78539816)],
                        theta=[0, degrees(pycircstat.mean(self.angles_ordered_a[:-1],
                                                          self.normed_spikes_per_segment[select[y * nr_cols + x]][:-1],
                                                          d=0.78539816))],
                        mode='lines',
    ...
    def present_rawspikes(self, cell_indices: list, trial_duration=4, sampling_freq=17852.76785):
        ...
        for cell_idx, cell_val, in enumerate(dat_cell_indices):
            ...
            for trigger in range(self.repeat_logic):
                spiketrain_list = []
                spikes_per_dir = (sas.get_spikes_per_trigger_type_new(
                    self.df_spikes.loc[cell_val]['Spikes'].compressed(),
                    ((self.df_stimulus['Trigger_Fr_relative'].values)[0]),
                    trigger, self.repeat_logic)[0])
            ...
            plotted_ds = self.plot_rawspikes(spiketrains_list, self.df_spikes.loc[cell_val][0], trial_duration,
                                             self.nr_trials)
